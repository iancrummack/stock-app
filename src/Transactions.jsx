...
export default function Transactions() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data, error } = await supabase
        ...
        .select('id, created_at, movement_type, quantity, product_id, location_id, project_id, created_by, our_order_ref, supplier_order_ref, products(code, name), locations(code, name), projects(code, name)')
  ...
  const visible = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (fRef.trim()) {
        const q = fRef.trim().toLowerCase()
        const hay = `${r.our_order_ref || ''} ${r.supplier_order_ref || ''}`.toLowerCase()
        if (!hay.includes(q)) return false
      }
      return true
    })
  }, [rows, fProductSearch, fType, fFrom, fTo, fWhere, fRef])

  function clearFilters() {
    setFProductSearch(''); setFType(''); setFFrom(''); setFTo(''); setFWhere(''); setFRef('')
  }
  const anyFilter = fProductSearch || fType || fFrom || fTo || fWhere || fRef
  ...
  function exportToExcel() {
    const exportRows = visible.map((r) => ({
      ...
      Quantity: r.quantity,
      Where: whereTo(r),
      'Our ref': r.our_order_ref || '',
      'Supplier ref': r.supplier_order_ref || '',
      By: whoDidIt(r),
    }))
    ...
    <div>
      ...
      <div className="filter-row">
        ...
        <input
          type="text"
          className="filter-search"
          placeholder="Search order ref (ours or supplier)"
          value={fRef}
          ...
          <tbody>
            {visible.map((r) => (
              <tr key={r.id}>
                ...
                <td>{whereTo(r)}</td>
                <td>{r.our_order_ref || '—'}</td>
                <td>{r.supplier_order_ref || '—'}</td>
                <td>{whoDidIt(r)}</td>
              </tr>
            ))}
          </tbody>
